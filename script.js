...
function buildCourse(fileList) {
  revoke(); const files = [...fileList]; if (!files.length) return; const root = files[0].webkitRelativePath?.split("/")[0] || "Local Course"; const states = saved(); const sectionMap = new Map();
...
function selectLesson(lesson, autoplay) {
  resetPreview(); $("#previewCard").hidden = true; selected = lesson; localStorage.setItem(`${STORAGE_KEY}-last`, lesson.id); const section = sectionFor(lesson.id), player = $("#videoPlayer"); $("#lessonHeader").hidden = false; $("#watchInfo").hidden = !lesson.video; $("#resourcesCard").hidden = !lesson.resources.length; $("#notesCard").hidden = false; $("#bookmarksCard").hidden = !lesson.video; $("#sectionName").textContent = `Section ${section.number}: ${section.title}`; $("#lessonTitle").textContent = lesson.title; $("#notes").value = lesson.notes || ""; updateCompleteButton(); renderResources(); renderBookmarks(); if (lesson.video) {
    ...
    player.onloadedmetadata = () => { errorBox.hidden = true; player.hidden = false; let targetTime = lesson.currentTime || 0; if (targetTime >= player.duration - 2) targetTime = 0; player.currentTime = targetTime; player.playbackRate = parseFloat($("#playbackSpeed").value); updateWatch(); if (autoplay) player.play().catch(() => { }) };
...
function exportProgress() {
  const data = {
    ...
    courseTitle: course?.title || "Tutorial Tracker Backup",
    progressData: saved(),
    lastLessonId: localStorage.getItem(`${STORAGE_KEY}-last`) || null
  };
  ...
  a.href = url;
  const safeName = (course?.title || "tutorial-progress").toLowerCase().replace(/[^a-z0-9]/g, "-").replace(/-+/g, "-");
...
function importProgress(file) {
  ...
  reader.onload = (e) => {
    try {
      ...
      const progressData = parsed.progressData || parsed;
      if (typeof progressData !== "object") throw new Error("No valid progress data found in file");
      
      const current = saved();
      const merged = { ...current, ...progressData };
      localStorage.setItem(STORAGE_KEY, JSON.stringify(merged));
      
      if (parsed.lastLessonId) {
        localStorage.setItem(`${STORAGE_KEY}-last`, parsed.lastLessonId);
      }
      ...
      if (course) {
        course.sections.forEach(s => {
          s.lessons.forEach(l => {
            if (merged[l.id]) {
              Object.assign(l, merged[l.id]);
              if (selected && selected.id === l.id) {
                Object.assign(selected, merged[l.id]);
              }
            }
          });
        });
        ...
        if (selected) {
          ...
          const activeMini = document.querySelector(".lesson.active .mini-progress i");
          if (activeMini) activeMini.style.width = `${selected.progress || 0}%`;
        }
      }
      
      alert("✅ Progress, notes, and bookmarks imported successfully!");
    } catch (err) {
      alert("❌ Could not import backup file: " + err.message);
    } finally {
...
$("#importDataInput")?.addEventListener("change", (e) => importProgress(e.target.files[0]));
